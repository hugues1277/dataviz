...
export const encryptionService = {
  ...
  decrypt(payload: string): string {
    ...
    if (buf.length < 29) {
      throw new Error(getServerMessage('exceptions.encryption.invalidPayloadTooShort'));
    }
    ...
    if (tag.length === 0) {
      throw new Error(getServerMessage('exceptions.encryption.invalidPayloadEmptyTag'));
    }
