...
/**
 * Classe pour gérer l'exécution de requêtes via Tauri
 */
class TauriQueryProvider implements IQueryProvider {
  /**
   * Exécute une requête via le proxy API Tauri
   */
  private async executeQueryProxy(connection: DBConnection, sql: string): Promise<QueryResult> {
    try {
      const response = await queryProxyUseCase.execute({ connectionId: connection.id, query: sql });
...
/**
 * Instance exportée du provider de requêtes Tauri
 */
export const tauriQueryProvider = new TauriQueryProvider();