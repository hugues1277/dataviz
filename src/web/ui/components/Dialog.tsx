...
const Dialog: React.FC<DialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-200 flex items-center justify-center p-4">
      ...
      {/* Modal Content */}
      <div
        className={`relative bg-[#111217] border ${currentTheme.border} w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200`}
      >
        <div className={`p-6 ${currentTheme.bg}`}>
          <div className="flex items-start gap-4">
            ...
            <button
              onClick={onClose}
              className="p-1 hover:bg-[#1f2127] rounded-full text-gray-500 transition-colors"
            >
              <X size={18} />
            </button>
          </div>
