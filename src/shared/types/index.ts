...
export function isDatabaseType(type: ConnectionType): boolean {
  return type === CONNECTION_TYPES.POSTGRES || type === CONNECTION_TYPES.MYSQL;
}

export function isApiType(type: ConnectionType): boolean {
...
export type DBConnectionConfig = Omit<
  DBConnection,
  "id" | "name" | "type"
>;
...
export type ChartVersionType =
  (typeof CHART_VERSION)[keyof typeof CHART_VERSION];
...
/** Position du label d'annotation : top | bottom pour axe X, left | right pour axe Y */
...
export interface DashboardVariable {
  id: string;
  /** Utilisé comme {{name}} dans les requêtes */
  name: string;
  label: string;
  type: VariableType;
  defaultValue: string;
